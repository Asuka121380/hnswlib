...
[[ "$stage" =~ ^(A0|A1|A2|B|C|D-quality|D-match|D-qps|D-tune-quality|D-tune-supplement-quality|D-tune-fine-quality|D-tune-select|D-tune-qps|D-target-qps|D-curve-baseline-quality|D-curve-candidate-quality)$ ]] || { echo 'invalid --stage' >&2; exit 2; }
...
if [[ "$stage" == D-quality || "$stage" == D-tune-quality || "$stage" == D-tune-supplement-quality || "$stage" == D-tune-fine-quality || "$stage" == D-curve-baseline-quality || "$stage" == D-curve-candidate-quality ]]; then
